...
if (!conn.authState.creds.registered) {
  ...
  console.log(
    chalk.green.bold(
      "[ Success ] Your Pairing Code : " + code?.match(/.{1,4}/g)?.join("-") ||
        code
    )
  );
}
...
async function connectionUpdate(update) {
  ...
  if (connection === "close") {
    ...
    if (code === DisconnectReason.loggedOut) {
      console.log(
        chalk.red(
          "❌ Session logged out. Silakan hapus folder sessions dan scan ulang pairing code."
        )
      );
    } else {
